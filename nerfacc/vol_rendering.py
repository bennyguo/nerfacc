...
def rendering_alpha(
    ...
    """Render the rays through the radience field defined by `rgb_alpha_fn`.

    This function is differentiable to the outputs of `rgb_alpha_fn` so it can be used for
    gradient-based optimization.
    ...
    Args:
        rgb_alpha_fn: A function that takes in samples {t_starts (N, 1), t_ends (N, 1), \
            ray indices (N,)} and returns the post-activation rgb (N, 3) and alpha \
            values (N, 1).
        packed_info: Packed ray marching info. See :func:`ray_marching` for details.
        t_starts: Per-sample start distance. Tensor with shape (n_samples, 1).
        t_ends: Per-sample end distance. Tensor with shape (n_samples, 1).
    ...
    .. code-block:: python
        ...
        def rgb_alpha_fn(t_starts, t_ends, ray_indices):
            # This is a dummy function that returns random values.
            rgbs = torch.rand((t_starts.shape[0], 3), device=device)
            alphas = torch.rand((t_starts.shape[0], 1), device=device)
            return rgbs, alphas
        colors, opacities, depths = rendering_alpha(rgb_alpha_fn, packed_info, t_starts, t_ends)
    ...
    # Query alpha and color with gradients
    ...
    ), "alphaas must have shape of (N, 1)! Got {}".format(alphas.shape)
